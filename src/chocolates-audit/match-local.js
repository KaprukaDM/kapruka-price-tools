// Matches Kapruka's Chocolates catalog against each competitor site's CACHED
// full catalogue (competitor_products). Includes this category's own
// CATALOG_ADAPTERS sites plus spar2u.lk, whose existing unfiltered
// Cosmetics-audit crawl already carries confectionery products (spot-checked:
// 198 chocolate-ish products before including).
//
// Same exact-model-code + fallback matching logic as
// electronics-audit/match-local.js — see ../compare/audit-scoring.js.
//
// Usage: node src/chocolates-audit/match-local.js [--limit=N] [--sites=domain1,domain2]
...
import { CATALOG_ADAPTERS as CHOCOLATE_ADAPTERS } from './site-adapters.js';
...
const KAPRUKA_SOURCE = { type: 'catalogue', buy: 'chocolates', subcat: null };

const REUSED_DOMAINS = new Set(['spar2u.lk']);

const ALL_SITES = [
  ...CHOCOLATE_ADAPTERS.map((s) => ({ domain: s.domain, name: s.name })),
