// Matches Kapruka's Home & Lifestyle catalog against each competitor site's
// CACHED full catalogue (competitor_products). Includes both this category's
// own CATALOG_ADAPTERS sites AND every site already crawled for Electronics/
// Cosmetics — those crawlers pulled entire unfiltered catalogues (WooCommerce
// Store API / Shopify products.json don't take a category filter), so sites
// like wasi.lk (Arpico) or dinapalagroup.lk already carry home/lifestyle
// products in their cached data with no fresh crawl needed.
//
// Same exact-model-code matching logic as electronics-audit/match-local.js.
//
// Usage: node src/home-lifestyle-audit/match-local.js [--limit=N] [--sites=domain1,domain2]
...
// Reused sites confirmed (by spot-check) to carry home/lifestyle products in
// their existing unfiltered catalogue crawl — matching against every
// Electronics/Cosmetics site would just waste time on categories with no
// overlap (pure computer-parts sites etc.), so this is a deliberate allowlist.
const REUSED_DOMAINS = new Set(['wasi.lk', 'dinapalagroup.lk', 'celltronics.lk']);

const ALL_SITES = [
  ...HOME_ADAPTERS.map((s) => ({ domain: s.domain, name: s.name })),
